...
Lifecycle migration preflight aborts on unknown status strings and reports each table, row ID, and value. Because an unknown value has no safe semantic mapping, repair it explicitly to one of the states documented in the API schema and rerun the migration. Duplicate active releases and platform domains are safe to normalize automatically: migration `003` keeps the newest release version and newest verified platform domain, retires the other rows, and reports each repair on standard error before installing the database constraints.
