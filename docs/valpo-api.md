...
`build.builder` selects an OCI builder image for that build target. `build.buildpacks` is a non-empty ordered array of builder-contained IDs or OCI buildpack images; duplicates are rejected. Omitted update fields retain existing values; explicit `null` resets the builder to the server default or the list to repository/builder detection. These fields are incompatible with `dockerfile` strategy. Selecting Dockerfile strategy clears inherited buildpack settings unless incompatible values are explicitly supplied. Manifest builds expose the same fields. An explicit list overrides buildpack selection in `project.toml`, with a job event explaining precedence. Successful buildpack release metadata includes resolved `builder` and `run_image` digests, `platform`, and detected buildpack versions. Configuration changes invalidate incompatible caches.
...
- Mutating endpoints that enqueue work accept an optional `Idempotency-Key` header (maximum 255 bytes). Repeating the same method, path, query, and body with the same key returns the original job, including after it finishes. Reusing a key for different request semantics returns `409`.
