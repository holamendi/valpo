...
ssh "$ssh_target" 'test ! -e /opt/valpo &&
  ...
  ! docker network inspect valpo >/dev/null 2>&1 &&
  test -z "$(docker ps -aq --filter label=valpo.managed=true)" &&
  test -z "$(docker volume ls -q | grep "^valpo-" || true)" &&
  test -z "$(docker image ls --filter "reference=valpo/*" -q)" &&
